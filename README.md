...
# Directory apps/2
combine_tsv.py - had to change .save to .close but then it worked. I also updated my test files with this one. There were problems caused by using equals signs. I think that may have explained all the previous problems where there was an error. This script now makes a spreadsheet, but it does have formatting in the first row.
